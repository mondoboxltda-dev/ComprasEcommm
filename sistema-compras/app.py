...
def ensure_auxiliary_registries():
    if using_postgres():
        ...
        category_sources = ["SELECT categoria nome FROM produtos WHERE categoria IS NOT NULL AND categoria<>''"]
        if column_exists("orcamentos", "categoria"):
            category_sources.append("SELECT categoria nome FROM orcamentos WHERE categoria IS NOT NULL AND categoria<>''")
        for row in query(" UNION ".join(category_sources)):
            execute(
                "INSERT INTO cadastro_categorias (nome,status) VALUES (%s,'ativo') ON CONFLICT (nome) DO NOTHING",
                (row["nome"],),
            )
        execute(
            "INSERT INTO cadastro_categorias (nome,status) VALUES (%s,'ativo') ON CONFLICT (nome) DO NOTHING",
        ...
        for nome in ("PIX", "Boleto", "Cartao de Credito", "Cartao de Debito", "Dinheiro", "Transferencia"):
            execute(
                "INSERT INTO cadastro_tipos_pagamento (nome,status) VALUES (%s,'ativo') ON CONFLICT (nome) DO NOTHING",
                (nome,),
            )
