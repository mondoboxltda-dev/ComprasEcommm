...
PERMISSION_ACTIONS = (
    ...
    ("reprovar", "Reprovar pedidos"),
    ("exportar", "Exportar dados"),
    ("visualizar_historico", "Visualizar historico"),
    ("gerenciar_usuarios", "Gerenciar usuarios"),
)
PERMISSION_COLUMNS = {name: f"pode_{name}" for name, _label in PERMISSION_ACTIONS}
ENDPOINT_MODULES = {
    "dashboard": "dashboard",
    "cadastros": "cadastros",
...
def ensure_auth_schema():
    ...
    if using_postgres():
        ...
        for action, _label in PERMISSION_ACTIONS:
            execute(f"ALTER TABLE usuario_permissoes ADD COLUMN IF NOT EXISTS pode_{action} SMALLINT NOT NULL DEFAULT 1")
        execute(
            """CREATE TABLE IF NOT EXISTS auditoria_sistema (
              ...
              registro_tipo VARCHAR(80) NULL,
              registro_id VARCHAR(80) NULL,
              informacao_anterior TEXT NULL,
              informacao_nova TEXT NULL,
              criado_em TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
            )"""
        )
    else:
        ...
        for action, _label in PERMISSION_ACTIONS:
            column = f"pode_{action}"
            if not column_exists("usuario_permissoes", column):
                execute(f"ALTER TABLE usuario_permissoes ADD COLUMN {column} TINYINT(1) NOT NULL DEFAULT 1")
        execute(
            """CREATE TABLE IF NOT EXISTS auditoria_sistema (
              ...
              registro_tipo VARCHAR(80) NULL,
              registro_id VARCHAR(80) NULL,
              informacao_anterior LONGTEXT NULL,
              informacao_nova LONGTEXT NULL,
              criado_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
              INDEX idx_auditoria_modulo_data (modulo,criado_em),
...
def get_user_permissions(user_id):
    columns = ",".join(PERMISSION_COLUMNS.values())
    rows = query(
        f"SELECT modulo,{columns} FROM usuario_permissoes WHERE usuario_id=%s",
        (user_id,),
    )
    return {
        row["modulo"]: {action for action, column in PERMISSION_COLUMNS.items() if row.get(column)}
        for row in rows
    }


def can(module, action="visualizar"):
    user = getattr(g, "current_user", None)
    if not module or not user:
        return False
    if user["tipo_acesso"] == "administrador":
        return True
    return action in getattr(g, "user_permissions", {}).get(module, set())


def audit_log(module, action, registro_tipo=None, registro_id=None, anterior=None, novo=None):
    user = getattr(g, "current_user", None)
    if not user or user.get("tipo_acesso") == "administrador":
        return
    execute(
        """INSERT INTO auditoria_sistema
           (usuario_id,usuario_nome,modulo,acao,registro_tipo,registro_id,informacao_anterior,informacao_nova)
           VALUES (%s,%s,%s,%s,%s,%s,%s,%s)""",
        (user["id"], user["nome"], module, action, registro_tipo, str(registro_id or "") or None,
         str(anterior) if anterior is not None else None, str(novo) if novo is not None else None),
    )


def endpoint_module(endpoint):
    if endpoint == "exportar_tela":
        target = (request.view_args or {}).get("tela")
        if target == "cadastros" and request.args.get("modulo") == "orcamentos":
            return "orcamentos"
        return target
...
def audit_previous_state(module, endpoint, record_id):
    ...
    tables = {
        ...
        "produtos": "produtos",
        "excluir_produto": "produtos",
        "nova_ordem": "ordens_compra",
        "excluir_ordem": "ordens_compra",
        "autorizacoes": "autorizacoes",
        "recebimentos": "recebimentos",
        "usuarios": "usuarios",
        "excluir_orcamento": "orcamentos",
    ...
    if endpoint in ("cadastros", "excluir_cadastro"):
        target = request.form.get("modulo") or request.args.get("modulo")
        table = "cadastro_tipos_pagamento" if target == "pagamentos" else "cadastro_categorias"
    if not table:
        return None
    row = query(f"SELECT * FROM {table} WHERE id=%s", (record_id,), one=True)
    if row and "senha_hash" in row:
        row = {key: value for key, value in row.items() if key != "senha_hash"}
    return row


def can_access(module):
    return can(module, "visualizar")
...
def can_view_any_history():
    return any(can(module, "visualizar_historico") for module, _label in PERMISSION_MODULES)
...
def require_login():
    ...
    g.current_user = user
    g.user_permissions = {module: {action for action, _label in PERMISSION_ACTIONS} for module, _label in PERMISSION_MODULES} if user["tipo_acesso"] == "administrador" else get_user_permissions(user["id"])
    ...
    action = "visualizar"
    if request.endpoint in ("exportar_tela", "exportar_historico_compras"):
        action = "exportar"
    elif request.method == "POST":
        if request.endpoint == "autorizacoes":
            action = "reprovar" if request.form.get("status") == "reprovada" else "aprovar"
        elif request.endpoint == "usuarios":
            action = "gerenciar_usuarios"
        elif request.endpoint and (request.endpoint.startswith("excluir_") or request.form.get("action") == "delete"):
            action = "excluir"
        elif request.form.get("id") or request.view_args and request.view_args.get("item_id"):
            action = "editar"
        else:
            action = "adicionar"
    if module and action != "visualizar" and not can(module, action):
        if request.path.startswith("/api/"):
            return jsonify({"error": "Voce nao possui permissao para esta acao."}), 403
        flash("Voce nao possui permissao para realizar esta acao.", "warning")
        return redirect(request.referrer or first_allowed_url())
    if module and (request.method == "POST" or action == "exportar"):
        source = request.form if request.method == "POST" else request.args
        safe_form = {key: value for key, value in source.items() if "senha" not in key.lower()}
        record_id = request.form.get("id") or (request.view_args or {}).get("item_id")
        g.pending_audit = (module, action, record_id, audit_previous_state(module, request.endpoint, record_id), safe_form)
    return None


@app.after_request
def register_request_audit(response):
    pending = getattr(g, "pending_audit", None)
    if pending and response.status_code < 400:
        module, action, record_id, old_data, new_data = pending
        audit_log(module, action, module, record_id, old_data, new_data)
    return response
...
def inject_globals():
    return {
        "hoje": date.today(),
        "ano": date.today().year,
        "current_module": endpoint_module(request.endpoint),
        "current_user": getattr(g, "current_user", None),
        "permission_modules": PERMISSION_MODULES,
        "permission_actions": PERMISSION_ACTIONS,
        "can_access": can_access,
        "can_view_any_history": can_view_any_history,
        "can": can,
    }
...
def usuarios():
    if not can("usuarios", "gerenciar_usuarios"):
        flash("Voce nao possui permissao para gerenciar usuarios.", "warning")
        return redirect(first_allowed_url())
    if request.method == "POST":
        form = request.form
        ...
        selected_permissions = {module: set() for module, _label in PERMISSION_MODULES}
        permission_levels = {
            "nao_exibir": set(),
            "visualizar": {"visualizar"},
            "adicionar": {"visualizar", "adicionar"},
            "editar": {"visualizar", "editar"},
            "completo": {action for action, _label in PERMISSION_ACTIONS},
        }
        for module, _label in PERMISSION_MODULES:
            level = form.get(f"nivel_{module}") or "nao_exibir"
            selected_permissions[module] = set(permission_levels.get(level, set()))
        if not nome or not login_name:
            flash("Nome e usuario/login sao obrigatorios.", "danger")
            return redirect(url_for("usuarios"))
        if tipo_acesso == "administrador":
            selected_permissions = {
                module: {action for action, _label in PERMISSION_ACTIONS}
                for module, _label in PERMISSION_MODULES
            }
        ...
        permission_columns = list(PERMISSION_COLUMNS.values())
        for module, _label in PERMISSION_MODULES:
            actions = selected_permissions.get(module, set())
            values = [1 if action in actions else 0 for action, _label in PERMISSION_ACTIONS]
            execute(
                f"INSERT INTO usuario_permissoes (usuario_id,modulo,{','.join(permission_columns)}) "
                f"VALUES (%s,%s,{','.join(['%s'] * len(permission_columns))})",
                (target_user_id, module, *values),
            )
        return redirect(url_for("usuarios"))

    users = query("SELECT id,nome,login,tipo_acesso,status,ultimo_login,criado_em,atualizado_em FROM usuarios ORDER BY nome")
    permissions = query(f"SELECT usuario_id,modulo,{','.join(PERMISSION_COLUMNS.values())} FROM usuario_permissoes")
    permissions_by_user = {}
    for row in permissions:
        permissions_by_user.setdefault(row["usuario_id"], {})[row["modulo"]] = {
            action for action, column in PERMISSION_COLUMNS.items() if row.get(column)
        }
...
@app.get("/auditoria")
def auditoria():
    if not can_view_any_history():
        flash("Voce nao possui permissao para visualizar o historico de alteracoes.", "warning")
        return redirect(first_allowed_url())
    filters = {
    ...
    where, params = [], []
    if filters["modulo"]:
        where.append("modulo=%s"); params.append(filters["modulo"])
    if filters["acao"]:
        where.append("acao=%s"); params.append(filters["acao"])
    if filters["usuario"]:
        where.append("usuario_nome LIKE %s"); params.append(f"%{filters['usuario']}%")
    if filters["data_inicio"]:
        where.append("criado_em >= %s"); params.append(filters["data_inicio"])
    if filters["data_fim"]:
        where.append("criado_em < %s")
        params.append((datetime.strptime(filters["data_fim"], "%Y-%m-%d") + timedelta(days=1)).strftime("%Y-%m-%d"))
    permitted_modules = [module for module, _label in PERMISSION_MODULES if can(module, "visualizar_historico")]
    if getattr(g, "current_user", {}).get("tipo_acesso") != "administrador":
        where.append("modulo IN (" + ",".join(["%s"] * len(permitted_modules)) + ")")
        params.extend(permitted_modules)
    where_sql = "WHERE " + " AND ".join(where) if where else ""
    rows = query(f"SELECT * FROM auditoria_sistema {where_sql} ORDER BY criado_em DESC,id DESC LIMIT 500", tuple(params))
    return render_template("auditoria.html", registros=rows, filtros=filters, modules=PERMISSION_MODULES, actions=PERMISSION_ACTIONS)


def export_rows_xlsx(rows, filename, sheet_name="Dados", columns=None):
    """Gera uma planilha simples somente com as linhas recebidas."""
    workbook = Workbook()
    ...
    if columns:
        ...
        keys = [key for key in rows[0].keys() if key != "senha_hash"]
        headers = keys
    else:
        keys, headers = [], ["Sem registros para os filtros aplicados"]
...
@app.get("/exportar/<tela>")
def exportar_tela(tela):
    """Exporta exclusivamente a listagem da tela atual, respeitando seus filtros GET."""
    modules = {
        "dashboard": "dashboard",
        "cadastros": "cadastros",
        "fornecedores": "fornecedores",
    ...
    module = modules.get(tela)
    if not module or not can(module, "exportar"):
        flash("Voce nao possui permissao para exportar esta tela.", "warning")
        return redirect(first_allowed_url())

    if tela == "historico_compras":
        return exportar_historico_compras()

    if tela == "fornecedores":
        rows = query("SELECT nome,cnpj,contato,telefone,email,status FROM fornecedores ORDER BY nome")
        return export_rows_xlsx(rows, "fornecedores", "Fornecedores")
    ...
    if tela == "produtos":
        ...
        where_sql = "WHERE " + " AND ".join(where) if where else ""
        rows = query(
            f"""SELECT p.codigo,p.descricao produto,p.categoria,u.nome unidade_compra,
                       p.quantidade_por_unidade_compra,p.unidade_base,f.nome fornecedor,
                       p.estoque_seguranca,p.unidade_estoque_seguranca,p.status,
                ...
                {where_sql} ORDER BY p.descricao""",
            tuple(params),
        )
        return export_rows_xlsx(rows, "produtos", "Produtos")

    if tela == "cadastros":
        module_name = request.args.get("modulo", "categorias")
        if module_name == "orcamentos":
            context = build_budget_context()
            return export_rows_xlsx(context["budget_rows"], "orcamentos", "Orcamentos", [
                ("Mes", "mes"), ("Ano", "ano"), ("Categoria", "categoria"),
                ("Orcamento", "valor_orcamento"), ("Gasto", "valor_gasto"),
                ("Saldo", "saldo"), ("Percentual", "percentual"), ("Status", "status"),
            ])
        table = "cadastro_tipos_pagamento" if module_name == "pagamentos" else "cadastro_categorias"
        search = (request.args.get("q") or "").strip()
        where = "WHERE nome LIKE %s" if search else ""
        params = (f"%{search}%",) if search else ()
        rows = query(f"SELECT nome,status,criado_em,atualizado_em FROM {table} {where} ORDER BY nome", params)
        filename = "tipos_pagamento" if module_name == "pagamentos" else "categorias"
        return export_rows_xlsx(rows, filename, filename.replace("_", " ").title())
    ...
    if tela == "ordens":
        ...
        where, params = [], []
        if numero:
            where.append("o.numero_oc LIKE %s")
            params.append(f"%{numero}%")
        if data_inicio:
            where.append("o.data_preenchimento >= %s")
        ...
        where_sql = "WHERE " + " AND ".join(where) if where else ""
        rows = query(
            f"""SELECT o.numero_oc,f.nome fornecedor,o.data_preenchimento,o.data_entrega,
                      o.metodo_pagamento,o.parcelas,o.prazos_parcelas,o.valor_total,o.status_compra
               FROM ordens_compra o JOIN fornecedores f ON f.id=o.fornecedor_id
               {where_sql} ORDER BY o.id DESC""",
            tuple(params),
        )
        return export_rows_xlsx(rows, "ordens_compra", "Ordens de compra")

    if tela == "autorizacoes":
        numero = (request.args.get("numero") or "").strip()
        data_inicio = request.args.get("data_inicio") or ""
        data_fim = request.args.get("data_fim") or ""
        where, params = [], []
        ...
        rows = query(
            f"""SELECT o.numero_oc,f.nome fornecedor,o.data_preenchimento,o.data_entrega,
                      o.valor_total,a.status,a.autorizado_por,a.data_aprovacao,a.decidido_em,a.observacao
               FROM autorizacoes a
        ...
        return export_rows_xlsx(rows, "autorizacoes", "Autorizacoes")

    if tela == "recebimentos":
        filters = {
            "origem": request.args.get("origem") or "",
            "fornecedor": request.args.get("fornecedor") or "",
            "produto": request.args.get("produto") or "",
        ...
        if filters["data_fim"]:
            ...
            params.append(filters["data_fim"])
        if filters["produto"]:
            where.append("""EXISTS (SELECT 1 FROM nfe_importacao_itens ix LEFT JOIN produtos px ON px.id=ix.produto_id WHERE ix.nfe_importacao_id=n.id AND (ix.descricao LIKE %s OR px.descricao LIKE %s OR px.codigo LIKE %s))""")
            term = f"%{filters['produto']}%"
            params.extend([term, term, term])
        where_sql = "WHERE " + " AND ".join(where) if where else ""
        rows = query(
            f"""SELECT n.data_entrada,n.fornecedor,n.fornecedor_cnpj,n.numero_nf,o.numero_oc,
                       n.valor_total,n.origem,n.status_recebimento,n.criado_em
                FROM nfe_importacoes n LEFT JOIN ordens_compra o ON o.id=n.ordem_id
                {where_sql} ORDER BY n.criado_em DESC,n.id DESC""",
            tuple(params),
        )
        return export_rows_xlsx(rows, "recebimentos", "Recebimentos")

    if tela == "usuarios":
        if getattr(g, "current_user", {}).get("tipo_acesso") != "administrador":
            flash("Apenas administradores podem exportar usuarios.", "warning")
            return redirect(first_allowed_url())
        rows = query("SELECT nome,login,tipo_acesso,status,ultimo_login,criado_em,atualizado_em FROM usuarios ORDER BY nome")
        return export_rows_xlsx(rows, "usuarios", "Usuarios")

    rows = query(
        """SELECT h.data_entrada,p.codigo,p.descricao produto,h.fornecedor,h.documento,
                  h.quantidade,h.unidade_compra,h.valor_unitario,h.valor_total,h.origem
           FROM historico_custos h JOIN produtos p ON p.id=h.produto_id
           ORDER BY h.data_entrada DESC,h.id DESC LIMIT 10"""
    )
    return export_rows_xlsx(rows, "dashboard", "Ultimas compras")


@app.get("/administracao/exportar-dados")
def exportar_dados():
    if getattr(g, "current_user", {}).get("tipo_acesso") != "administrador":
        flash("Apenas administradores podem exportar os dados.", "warning")
        return redirect(first_allowed_url())

    datasets = (
        ("fornecedores", "SELECT * FROM fornecedores ORDER BY id"),
        ("produtos", "SELECT * FROM produtos ORDER BY id"),
        ("ordens_compra", "SELECT * FROM ordens_compra ORDER BY id"),
        ("ordem_itens", "SELECT * FROM ordem_compra_itens ORDER BY id"),
        ("autorizacoes", "SELECT * FROM autorizacoes ORDER BY id"),
        ("recebimentos", "SELECT * FROM recebimentos ORDER BY id"),
        ("nfe_importacoes", "SELECT * FROM nfe_importacoes ORDER BY id"),
        ("nfe_itens", "SELECT * FROM nfe_importacao_itens ORDER BY id"),
        ("movimentacoes", "SELECT * FROM movimentacoes_estoque ORDER BY id"),
        ("historico_custos", "SELECT * FROM historico_custos ORDER BY id"),
        ("recebimento_auditoria", "SELECT * FROM recebimento_auditoria ORDER BY id"),
        ("orcamentos", "SELECT * FROM orcamentos ORDER BY id"),
        ("categorias", "SELECT * FROM cadastro_categorias ORDER BY id"),
        ("tipos_pagamento", "SELECT * FROM cadastro_tipos_pagamento ORDER BY id"),
        ("unidades_medida", "SELECT * FROM unidades_medida ORDER BY id"),
        (
            "produto_fornecedor",
            "SELECT * FROM produto_fornecedor_relacionamentos ORDER BY id",
        ),
        (
            "usuarios",
            """
            SELECT id,nome,login,tipo_acesso,status,ultimo_login,criado_em,atualizado_em
            FROM usuarios ORDER BY id
            """,
        ),
        (
            "usuario_permissoes",
            "SELECT * FROM usuario_permissoes ORDER BY id",
        ),
    )

    workbook = Workbook()
    workbook.remove(workbook.active)
    for sheet_name, sql in datasets:
        rows = query(sql)
    ...
    metadata = workbook.create_sheet("informacoes", 0)
    metadata.append(["ComprasPro - Exportacao completa"])
    metadata.append(["Gerado em", datetime.now().strftime("%d/%m/%Y %H:%M:%S")])
    metadata.append(["Gerado por", g.current_user["nome"]])
    metadata.append(["Observacao", "Hashes de senha nao sao incluidos por seguranca."])
    ...
    workbook.save(output)
    output.seek(0)
    filename = f"compraspro_dados_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
    return send_file(
        output,
        as_attachment=True,
...
def ordens():
    ...
    where_sql = "WHERE " + " AND ".join(where) if where else ""
    if using_postgres():
        rows = query(f"""SELECT o.*, f.nome fornecedor, oo.numero_oc ordem_original_numero,
                        ...
                        ORDER BY o.id DESC""", tuple(params))
    else:
        rows = query(f"""SELECT o.*, f.nome fornecedor, oo.numero_oc ordem_original_numero,
                        COUNT(i.id) total_itens,
                        ...
                        GROUP BY o.id
                        ORDER BY o.id DESC""", tuple(params))
    return render_template("ordens.html", ordens=rows, filtros=filters)
...
def autorizacoes():
    ...
    where_sql = "WHERE " + " AND ".join(where) if where else ""
    if using_postgres():
        rows = query(f"""SELECT a.*,o.numero_oc,o.valor_total,o.data_entrega,o.data_preenchimento,
                               f.nome fornecedor,
                        ...
                        ORDER BY a.id DESC""", tuple(params))
    else:
        rows = query(f"""SELECT a.*,o.numero_oc,o.valor_total,o.data_entrega,o.data_preenchimento,
                               f.nome fornecedor,
                               COALESCE(SUM(i.quantidade),o.quantidade) quantidade,
    ...
    return render_template("autorizacoes.html", autorizacoes=rows, filtros=filters)
