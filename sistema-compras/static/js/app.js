...
// Oculta no frontend as acoes negadas; o backend repete a validacao.
document.addEventListener('DOMContentLoaded', () => {
  const permissions = document.body.dataset;
  const allowed = key => permissions[key] === '1';
  const hide = element => { if (element) element.classList.add('d-none'); };
  document.querySelectorAll('form').forEach(form => {
    const action = (form.getAttribute('action') || '').toLowerCase();
    const formAction = form.querySelector('input[name="action"]')?.value || '';
    if (!allowed('canDelete') && (action.includes('/excluir') || formAction === 'delete')) hide(form);
  });
  document.querySelectorAll('button,a.btn').forEach(button => {
    const text = (button.textContent || '').trim().toLowerCase();
    const icon = button.querySelector('i');
    const iconClass = icon?.className || '';
    if (!allowed('canAdd') && (/^(novo|nova|adicionar|importar|cadastrar|criar)/.test(text) || iconClass.includes('bi-plus-lg'))) hide(button);
    if (!allowed('canEdit') && (iconClass.includes('bi-pencil') || /^(editar|salvar alteracao)/.test(text))) hide(button);
    if (!allowed('canApprove') && /^aprovar/.test(text)) hide(button);
    if (!allowed('canReject') && /^reprovar/.test(text)) hide(button);
  });
});
