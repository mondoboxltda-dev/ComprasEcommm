...
def get_request_db():
    ...
    if using_postgres():
        if ConnectionPool is None:
            raise RuntimeError("psycopg_pool nao esta instalado. Rode: pip install -r requirements.txt")
        if POSTGRES_POOL is None:
            POSTGRES_POOL = ConnectionPool(
                conninfo=app.config["DATABASE_URL"],
                min_size=app.config["DB_POOL_MIN_SIZE"],
                max_size=app.config["DB_POOL_MAX_SIZE"],
...
def close_request_db(_exception=None):
    ...
    try:
        ...
        pass
    if using_postgres() and POSTGRES_POOL is not None:
